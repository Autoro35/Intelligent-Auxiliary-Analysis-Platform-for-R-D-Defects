...
**状态**：构建通过、接口契约全部通过，但**所有 Vue 页面从未在真实浏览器渲染过**（开发机无浏览器自动化工具）。

**已验证**：`npm run build` 全量通过、45 个接口契约全通过、dev server 与 `/api` 代理可用。
**未验证**：侧边栏按角色过滤菜单、路由守卫跳转、表单校验交互、ECharts 图表渲染、附件上传下载的浏览器行为。

**验收方法**：按 [docs/开发与部署文档.md](docs/开发与部署文档.md) 第三节启动前后端，用 `admin` / `admin123` 登录后逐个页面点击一遍。

---

## 四、其他待改进项

### 4.1 JWT 无状态导致禁用/删除用户的令牌在过期前仍有效
